...
function Options({ question, dispatch, answer,width }) {
  ...
  return (<div>
<div className="flex flex-col m-4"><div className={`${width<1000?"sm":"quest"}`}>Category: {question.category}</div>{width<1000?<div>{diffDic[question["difficulty"]]}</div>:<div className="quest">Difficulty:&nbsp;{question["difficulty"]}</div>}</div>
